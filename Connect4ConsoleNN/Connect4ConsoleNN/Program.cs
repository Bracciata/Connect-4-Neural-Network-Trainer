...
namespace Connect4ConsoleNN
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (training)
            {
                Console.Write("How many times would you like to train the network? ");

                int numberTrains = Convert.ToInt32(Console.ReadLine());
                int playerOneWins = 0;
                int playerTwoWins = 0;
                int catGame = 0;
                NeuralNetworkOwn nn = new NeuralNetworkOwn(42, 2, 70, 42);
                for (int i = 0; i < numberTrains; i++)
                {
                    Connect4 c = new Connect4();
                    bool playerOneTurn = true;
                    int checkWinResult = -2;
                    while (checkWinResult == -2)
                    {
                        //find optimal move
                        // TODO create optimal move finder may need to have seperate one for both players if player two is based off of old player one
                        int[] usedBy = c.getUsedBy();
                        double[] moveValues = nn.getMoveValues(usedBy,playerOneTurn); //replace with find best move choices
                        //check usable spots
                        bool[] spacesUsable = c.gameCheckPeices();
                        //choose move
                        c.makeMove(playerOneTurn, moveValues, spacesUsable);
                        playerOneTurn = !playerOneTurn;
                        checkWinResult = c.checkWin();
                    }
                    ...
                    int[] usedByToSend = c.getUsedBy();

                    nn.trainNeuralNetwork(result, usedByToSend);
                    //below can be deleted especially considering it may become messy.
                    if (result == -1)
                    {
                        Console.WriteLine("Trial: " + (i + 1).ToString() + " player one won!");
                        playerOneWins += 1;
                    }
                    else if (result == 1)
                    {
                        Console.WriteLine("Trial: " + (i + 1).ToString() + " player two won!");
                        playerTwoWins += 1;
                    }
                    else
                    {
                        Console.WriteLine("Trial: " + (i + 1).ToString() + " is a cat game!");
                        catGame += 1;
                    }
                    Console.WriteLine(c.getFails().ToString());

                }
                Console.WriteLine("Training Complete. Results for the " + numberTrains.ToString() + " games: I won " + playerOneWins.ToString() + " games, old me won " + playerTwoWins.ToString() + " games, and we tied " + catGame.ToString() + " times!");
                Console.Write("Would you like to continue training. Yes or No? ");
                if (Console.ReadLine().Contains("Y") || Console.ReadLine().Contains("y"))
                {
                    //no need for anything here because training was going to continue anyways
                }
                else
                {
                    //end training even though the grind never stops
                    training = false;
                }
            }
        }
   ...
   //2 hiddne layers, 70 neurons each layer (2/3*42)+42
    public class NeuralNetworkOwn
    {
        ...
        public void saveToDatabase()
        {
            ...
            string dir = folderOfProgram.Substring(0, folderOfProgram.Length - 29);
